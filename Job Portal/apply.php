...
if (!isLoggedIn()) {
    setMessage('error', 'You must be logged in to apply for jobs');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !$alreadyApplied) {
    ...
    if (empty($error)) {
        $insertQuery = "INSERT INTO applications (job_id, user_id, cover_letter, contact_phone, status, applied_date) 
                        VALUES (?, ?, ?, ?, 'Pending', NOW())";
        $stmt = $conn->prepare($insertQuery);
        $stmt->bind_param("iiss", $jobId, $userId, $coverLetter, $phone);
